...
You already test, fuzz, and review your C. Those tools sample behavior: they run the code on some inputs and look for failures. **QCP (Qualified C Programming)** does something different — it **proves** that a function meets its specification for *all* inputs in scope. Where a test says "no failure on the cases I tried," a QCP proof says "no failure, by construction, against the spec I wrote." **You write the specification; QCP's automated solver and (optionally) an LLM produce the proof.** That proof is expressed in separation logic, and it is **two-tier**: the routine parts are discharged by QCP's strategy solver and trusted, while the parts that end in a manual `Qed` are re-checked by the Rocq (formerly Coq) proof assistant — but **you do not need to know either layer to start.** (When you *do* want to weigh what the green check is worth, [ch 10](ch10-trust-and-soundness.md) gives that two-tier split its full weight.)
...
**"Manual" does not mean "you write it by hand."** QCP splits the proof work into an *auto* fraction the strategy solver closes by itself and a *manual* fraction it can't — and on the current corpus snapshot the manual fraction is **roughly a quarter to a third of proof effort** (a snapshot from regenerated artifacts that drifts; [ch 12](ch12-scope-and-scaling.md) carries the command-first counts). But *manual* is QCP's name for "needs a written Rocq proof," **not** "a human must write it." With the AI dial up, the **LLM drafts those proofs** (and the loop invariants); your residual job is to *review* them and to step in only on the rare, genuinely hard cases the LLM can't close. So a tier-1 practitioner's hands-on proving burden is much smaller than "a third" — it's whatever the LLM leaves behind, which for routine array/string/list code reusing the shipped predicates is little. (How small depends on the case and the model — this is frontier-model territory today, [ch 7](ch07-invariants-and-the-ai-dial.md).)
...
5. **Check.** `coqc` compiles the Rocq files against the `SeparationLogic/` library; the `Qed` bodies are re-checked by the kernel, and a `goal_check` module confirms every VC is accounted for.
...
The **CLI core is battle-tested**; **QIDE is shipped** for live annotation. The MCP/AI surface is partly **intended-workflow**: it needs a frontier model today, carries real setup friction, and has Windows gaps — [ch 7](ch07-invariants-and-the-ai-dial.md) covers it honestly, with the friction named.
