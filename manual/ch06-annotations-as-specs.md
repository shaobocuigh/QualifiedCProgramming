...
**`Assert`** pins the state at one spot. `symexec` records a verification condition (a **VC** — an entailment that the strategy solver discharges automatically, or that a written Rocq (formerly Coq) proof closes and the kernel re-checks at `Qed`) that the *prior* state implies your asserted state, then continues from your assertion. Use it to re-shape the state into the form a later step needs.
