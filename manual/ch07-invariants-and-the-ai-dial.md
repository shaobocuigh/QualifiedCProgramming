...
> **Note (maturity):** the CLI / `symexec` / `coqc` core (Stage A) is **battle-tested**. The MCP/AI layer in this chapter is **largely intended-workflow**: it is designed and shipped, but it carries real friction (frontier-model requirement, setup cost, Windows gaps, a Chinese contract file) — none a reason to avoid it, all a reason to budget for it. The friction is laid out in full at ["When the tooling crashes"](#when-the-tooling-crashes-not-when-the-proof-is-stuck) below. Where a claim here is intended-workflow rather than corpus-proven, it is marked.
...
Setup, env-var resolution, Python floors, and copy-paste editor registration live in `docs/mcp-servers.md` and `README_LINUX.md` — install once, then drive the agent, not the tools.
...
*The agent workflow's phase state machine: forward through the phases, looping back to `annotation` whenever a downstream phase exposes a too-weak spec or an unprovable obligation. (Mermaid state IDs cannot contain hyphens, so the diagram renders `goal_frozen`/`vc_checking`/`vc_proving`/`final_check` with underscores; the prose and table below use the canonical hyphenated names.)*
...
The design enforces strict file ownership: sub-agents are read-only by default and write only their assigned scratch; only the **main agent** writes the case's master-state files and runs `symexec`/`coqc`. The completion criteria include the trust-relevant gate — **no `Admitted` or extra `Axiom` in the manual file or case lib** ([ch 10](ch10-trust-and-soundness.md)'s one real watch-item, enforced as a `done` condition). The full contract lives in `AGENTS.md` (Chinese); the English summary — including the per-phase detail, the seven skills, and the deprecated combined skill — is `docs/agent-workflow.md`.

> **Honest limit:** `AGENTS.md` being Chinese is a real onboarding tax for an English-speaking operator — read the English `docs/agent-workflow.md` summary alongside it, not instead of it for the authoritative details.
...
The shipped agent scripts are intended-workflow and can fail as software — e.g. a known `vc-proving` `NameError` that can leave `Admitted` stubs in `*_proof_manual.v` and so masquerade as "N unproven obligations" rather than a tool fault. The details, the self-check recipe (`pyflakes` for undefined-name bugs; a `grep` for leftover `Admitted`), and the two-line fix are in [ch 13](ch13-honest-limits.md) / [R5](reference/TROUBLESHOOTING.md) and `docs/agent-workflow.md` §9.1. After **any** tool failure, the question to answer is just: did the tool finish, or did it crash and leave a hole behind? — and that audit is exactly why [ch 10](ch10-trust-and-soundness.md) makes a manual-file `Admitted` the one thing worth actively auditing.
...
- **An LLM `Qed` is kernel-checked** — but an LLM can "fix" a goal by weakening the spec. Review the WHAT; the kernel only guards the WHY. Full trust story: [ch 10](ch10-trust-and-soundness.md). Stuck AI: [ch 11](ch11-stuck-goal-differential.md) cause 4.
...
- **Budget for the friction honestly:** frontier model required, real setup cost, `rocq-mcp` interactive proving unavailable on Windows, `AGENTS.md` in Chinese, and shipped agent scripts that can crash (a patchable `vc-proving` glitch among them). None is a dealbreaker; all are intended-workflow.
