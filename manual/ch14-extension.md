...
> **Note:** Read this chapter as the by-example *workflow* for adding a predicate and its rules. The `.strategies` rule language itself — its grammar, the priority/check/action vocabulary, and what the solver does with a rule — is the subject of [Chapter 15 — the strategy DSL](ch15-strategy-dsl.md); reach for it when you want the language rather than the moves. Every form shown here is quoted from a real shipped `.strategies` file; **nothing is invented syntax**. The corpus is your pattern catalogue (start with `QCP_examples/QCP_demos_human/sll.strategies`, `int_array.strategies`, and `bst.strategies`); the formal specification is the Stellis paper (arXiv:2512.05159), cited in [ch 15](ch15-strategy-dsl.md).
...
The other moves you'll meet in the corpus are variations on these: `left_add`/`right_add` add a fact *or a predicate* to the selected side (a pure equality like `x0 == x1`, or a spatial predicate like `IntArray::missing_i` above — pure equalities are just the simplest case), the `left_exist_add`/`right_exist_add` variants introduce existentials, `check : infer(...)` guards a rule, and `priority` controls when a rule is tried. [Chapter 15](ch15-strategy-dsl.md) tabulates the full vocabulary — every priority class, both checks, and every action. Treat the shipped `.strategies` files as your pattern catalogue; do not extrapolate forms that don't appear there.
...
Most strategy proofs close with `Qed` and are kernel-checked; within the shipped strategy library two files (`string` and `minigmp`) carry a small `Admitted` residue that is **trusted, not proven** — the same trust-the-tool relationship the auto VCs have, and part of QCP's Trusted Computing Base. Out-of-tree example libraries you bring in (e.g. a vendored case under `examples/External/`) carry their own residue — audit those the same way. The exact count and the grep that finds them live in [ch 10](ch10-trust-and-soundness.md), which owns that audit.
...
- **Strategy soundness is the same two-tier trust as everything else**: a rule's `…_correctness` lemma is kernel-checked when you compile it and it ends in `Qed`. Most rules do; within the shipped library two files (`string`, `minigmp`) carry a small `Admitted` residue that is trusted, not proven (out-of-tree examples you import may carry more). Land *your* rules in the `Qed` majority, and audit them with the recipes in [ch 10](ch10-trust-and-soundness.md).
- **This chapter is the by-example workflow; the language reference is [ch 15](ch15-strategy-dsl.md)**: copy rule shapes from the corpus (`sll.strategies`, `int_array.strategies`, `bst.strategies`) and don't invent syntax. When a goal a strategy *should* have closed stays red, [ch 11 — the Stuck-Goal Differential](ch11-stuck-goal-differential.md) cause #4 (automation came up short) is the place to add or sharpen a rule.
